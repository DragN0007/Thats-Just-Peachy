...
public class TJPPlacedFeatures {
    public static final ResourceKey<PlacedFeature> PEACH = registerKey("peach");

    public static void bootstrap(BootstapContext<PlacedFeature> context) {
        HolderGetter<ConfiguredFeature<?, ?>> configuredFeatures = context.lookup(Registries.CONFIGURED_FEATURE);
        register(context, PEACH, configuredFeatures.getOrThrow(TJPConfigFeatures.PEACH),
                VegetationPlacements.treePlacement(PlacementUtils.countExtra(0, 0.01f, 1),
                        TJPBlocks.PEACH_SAPLING.get()));
    }

    public static ResourceKey<PlacedFeature> registerKey (String name){
        return ResourceKey.create(Registries.PLACED_FEATURE, new ResourceLocation(ThatsJustPeachy.MODID, name));
    }
